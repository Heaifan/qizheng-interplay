...
> 发布日期：2026-05-17
> Release：[v0.4.1.6](https://github.com/Heaifan/qizheng-interplay/releases/tag/v0.4.1.6)

### 修复
- **重新执行战斗时压制值未清零**：`resetSuppressionState()` 重置所有压制字段
- **重新执行战斗时弹药继承上一局**：`weaponState = createWeaponRuntimeState()` 重置满弹
- **日志显示原始命中率而非实效命中率**：新增 `受压×倍率｜实效 X%` 显示
- **`suppressionHitCount` 重命名**：改为 `suppressionEventCount`，报告文案改为「受压事件」
- **移除旧 `firePressure` 日志**：不再显示「火力压力」，新压制系统独立干净

### 调整
- `startExecution()` 现在重置压制状态、弹药状态、速度
- 战斗日志命中率改为：`命中率 50.0%｜受压×0.78｜实效 39.0%`
- 报告压制表使用 `peakSuppression` 计算最低命中倍率和最高射速惩罚
...
> 发布日期：2026-05-17
> Release：[v0.4.1.5](https://github.com/Heaifan/qizheng-interplay/releases/tag/v0.4.1.5)

### 新增
- **压制能力 MVP**：射击命中与未命中均可造成压制，压制值 0~1 随时间衰减
- `RuntimeUnit` 增加 `suppression` / `lastSuppressedAtMs` 字段
- `WeaponProfile` 增加 `suppressionPower` 字段
- **`suppression.ts`**：压制模块（计算增量、衰减、命中率/开火间隔倍率、状态标签）
- 单位档案新增「压制状态」卡片，显示压制值、状态标签、命中影响、开火间隔影响
- 战斗报告火力统计表增加「最终压制」列

### 调整
- **压制影响命中率**：压制 0→1 对应命中率 ×1.0 → ×0.55（线性衰减）
- **压制影响开火间隔**：压制 0→1 对应开火间隔 ×1.0 → ×1.60
- 战斗日志显示压制增量与目标当前压制值（`压制+0.07｜红方压制0.28`）
- 每 tick 衰减压制（每秒 0.08，完全恢复约 12.5s）
- 压制增量受距离、点射发数、命中/未命中加权（命中 ×1.25，未命中 ×0.70）

### 武器平衡
- Kar98k / M91/30 / 三八式：`suppressionPower = 0.025`（步枪单发压制低）
- M1 Garand：`suppressionPower = 0.035`（半自动略高）
- MP40：`suppressionPower = 0.055` / PPSh-41：`suppressionPower = 0.065`（冲锋枪中等）
- MG34：`suppressionPower = 0.120` / ZB26：`suppressionPower = 0.100`（机枪高压制）
